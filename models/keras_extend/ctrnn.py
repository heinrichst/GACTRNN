# -*- coding: utf-8 -*-
"""This tensorflow extension implements a CTRNN
in order to build CTRNNs with multiple modules.
Heinrich 2020
"""
...
@tf_export('keras.layers.SimpleCTRNNCell')
class SimpleCTRNNCell(Layer):
    """Cell class for SimpleCTRNNCell.
    ...
    def get_config(self):
        ...
        base_config = super(SimpleCTRNNCell, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))


@tf_export('keras.layers.SimpleCTRNN')
class SimpleCTRNN(RNN):
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        ...
        return super(SimpleCTRNN, self).call(
...
class CTRNNCell(Layer):
    ...
    def call(self, inputs, states, training=None):
        x = inputs          # for better readability
        prev_y = states[0]  # previous output state
#        prev_z = states[1][0]  # previous internal state
        prev_z = states[1]  # previous internal state
        #prev_t = states[2]  # previous internal state
        if 0 < self.dropout < 1 and self._dropout_mask is None:
            self._dropout_mask = _generate_dropout_mask(
                    array_ops.ones_like(x),
                    self.dropout,
        ...
        if _DEBUGMODE:
            x = gen_array_ops.check_numerics(x, 'ACTRNNCell: Numeric error for x')
            prev_y = gen_array_ops.check_numerics(prev_y, 'ACTRNNCell: Numeric error for prev_y')
            prev_z = gen_array_ops.check_numerics(prev_z, 'ACTRNNCell: Numeric error for prev_z')
            #prev_t = gen_array_ops.check_numerics(prev_t, 'ACTRNNCell: Numeric error for prev_t')
            h = gen_array_ops.check_numerics(h, 'ACTRNNCell: Numeric error for h')
            r = gen_array_ops.check_numerics(r, 'ACTRNNCell: Numeric error for r')
            y = gen_array_ops.check_numerics(y, 'ACTRNNCell: Numeric error for y')
            z = gen_array_ops.check_numerics(z, 'ACTRNNCell: Numeric error for z')
            print("shapes x", array_ops.shape(x))
            print("shapes prev_y", array_ops.shape(prev_y))
            print("shapes prev_z", array_ops.shape(prev_z))
            print("shapes h", array_ops.shape(h))
            print("shapes r", array_ops.shape(r))
            print("shapes y", array_ops.shape(y))
            print("shapes z", array_ops.shape(z))
            print("shapes t", array_ops.shape(t))
            print("shapes self.taus", array_ops.shape(self.taus))
    ...
    def get_taus(self):
        return self.taus if self.built else None
...
class CTRNN(RNN):
    """Continuous Time RNN that can have several modules
